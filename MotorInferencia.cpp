...
void MotorInferencia::setValoresInferencia() {
    for (int i = 0; i < reglas.size(); i++) 
    {
        ...
        std::cout << "Estableciendo valor de inferencia: " << reglas[i]->getValorInferencia() << " -> " << reglas[i]->getConsecuencia() << std::endl;
...
void MotorInferencia::inferir() {
    std::cout << "----- Iniciando Inferencia: -----" << std::endl;
    ...
    for (int i = 0; i < reglas.size(); i++) 
    {
        ...
        for (int j = 0; j < salida->getTerminos().size(); j++) 
        {
            if (salida->getTerminos()[j]->getNombre() == nombreConsecuente) 
            {
                ...
                {
                    std::cout << "Estableciendo altura en termino de salida: " << salida->getTerminos()[j]->getNombre() << " = " << valorInferencia << std::endl;
...
float MotorInferencia::defuzzificar(float paso) {
    std::cout << "----- Iniciando Defuzzificacion: -----" << std::endl;
    float numerador = 0;
    float denominador = 0;
    ...
    std::cout << "Valor de salida: " << (numerador / denominador) << std::endl;
